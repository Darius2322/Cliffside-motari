...
import { getVisibleStaff } from '@/lib/actions/staff';

export const metadata: Metadata = { title: 'About' };

export default async function AboutPage() {
  const staff = await getVisibleStaff();
  const leadership = staff.filter((s) => s.is_leadership);
  const generalStaff = staff.filter((s) => !s.is_leadership);
            ...
            <>
              {leadership.length > 0 && (
                <div className="mb-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                  {leadership.map((person) => (
                    <div key={person.id} className="rounded-sm border border-border bg-surface p-6">
                      {person.publicPhotoUrl && (
                        <Image
                          src={person.publicPhotoUrl}
                      ...
                      <h4 className="font-serif text-base text-canopy">{person.full_name}</h4>
                      <p className="text-sm text-mist">{person.position}</p>
                      {person.bio && <p className="mt-2 text-sm text-mist">{person.bio}</p>}
                    </div>
                  ))}
                </div>
              )}
              {generalStaff.length > 0 && (
                <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
                  {generalStaff.map((person) => (
                    <div key={person.id} className="rounded-sm border border-border bg-surface p-5">
                      {person.publicPhotoUrl && (
                        <Image
                          src={person.publicPhotoUrl}
                          alt={person.full_name}
                      ...
                      <h4 className="font-serif text-sm text-canopy">{person.full_name}</h4>
